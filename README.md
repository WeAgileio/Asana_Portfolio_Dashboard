...
- **畫面**：Notion 模式隱藏「十週更新統計」，專案進展與請款頁沿用既有畫面；請款金額與階段來自各專案的任務。
- **破壞性變更**：無。

### v1.11.1

- **月欄表對齊**：**請款進展·時間序**、**專案進展·時間序**月欄寬度固定，表頭請款金額與下方格線一致對齊；金額較長時可橫向捲動檢視。
- **破壞性變更**：無。

### v1.11.0

- **展示模式（Demo Mode）**：可建置獨立 demo 映像（`VITE_DEMO_MODE=true`），開啟即用虛構建案／請款資料，不需 PAT；隱藏「十週更新統計」；支援 `make demo` 本機驗證與 `docker-compose.demo.yml` 獨立部署。
- **月欄表對齊**：**請款進展·時間序**、**專案進展·時間序**表頭與內容月欄寬度一致，載入即對齊；請款金額完整顯示，依資料動態調整欄寬。
- **破壞性變更**：無。

### v1.10.10

- **共用專案選擇器**：**專案進展·時間序**、**請款進展**、**請款進展·時間序**改用 `ProjectPickerPanel`；支援專案名稱搜尋、創建時間篩選（近半年／1·3·5 年、年份、起迄）、建立／名稱排序；列表顯示建立日期；篩選僅影響顯示，已勾選但被藏起的專案仍保留；關閉面板重置篩選 UI。
- **全選**：可一次勾選目前篩選結果中的全部專案（合併至已選，不取消 hidden 已選）；列表空或已全部勾選時按鈕 disabled。
- **破壞性變更**：無。

### v1.10.9

- **三頁工具列可隱藏**：**專案進展·時間序**、**請款進展**、**請款進展·時間序**共用 `PageToolbar`；點「隱藏」後 header 不占高度，以偏右「顯示工具列」還原；偏好存入 `localStorage`；寬度不足且未手動顯示時自動隱藏，手動顯示後改橫向捲動。
- **專案進展·時間序 section 截止日**：進行中區段以**最早未完成任務** `due_on` 作月欄歸位與卡片「截止日期」；全部完成時以**最晚有 due 任務**；皆無 due 則未排。狀態色（延後／風險等）仍依 milestone 計算。
- **破壞性變更**：無。
